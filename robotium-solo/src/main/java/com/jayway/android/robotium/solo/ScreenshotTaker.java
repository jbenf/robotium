...
/**
 * Contains screenshot methods like: takeScreenshot(final View, final String
 * name), startScreenshotSequence(final String name, final int quality, final
 * int frameDelay, final int maxFrames), stopScreenshotSequence().
 * 
 * 
...
class ScreenshotTaker {
	 ...
	 */
	public void takeScreenshot(final String dirPath, final String name, final int quality) {
		View decorView = getScreenshotView();
		if (decorView == null)
			return;

		initScreenShotSaver();
		ScreenshotRunnable runnable = new ScreenshotRunnable(decorView, dirPath, name, quality);
	...
	public static String defaultScreenshotDir() {
		return Environment.getExternalStorageDirectory() + "/Robotium-Screenshots/";
	}

	/**
	 * This is the thread which causes a screenshot sequence to happen in
	...
	private class ScreenshotSequenceThread extends Thread {
		...
		public void doScreenshot() {
			...
			ScreenshotRunnable r = new ScreenshotRunnable(v, defaultScreenshotDir(), final_name, quality);
	...
	private class ScreenshotRunnable implements Runnable {
		...
		public ScreenshotRunnable(final View _view, final String _dirPath, final String _name, final int _quality) {
			view = _view;
			name = _name;
			dirPath = _dirPath;
			quality = _quality;
		...
		public void run() {
			if (view != null) {
				...
				if (b != null)
					screenShotSaver.saveBitmap(b, dirPath, name, quality);
				else
	...
	private class ScreenShotSaver extends Handler {
		 ...
		 */
		public void saveBitmap(Bitmap bitmap, String directory, String name, int quality) {
			Message message = this.obtainMessage();
			message.arg1 = quality;
			message.obj = bitmap;
			message.getData().putString("name", name);
			message.getData().putString("dir", directory);
		...
		public void handleMessage(Message message) {
			...
			String dir = message.getData().getString("dir");
			int quality = message.arg1;
			Bitmap b = (Bitmap) message.obj;
			if (b != null) {
				saveFile(dir, name, b, quality);
				b.recycle();
			} else {
				Log.d(LOG_TAG, "NULL BITMAP!!");
			}
		 ...
		 * 
		 */
		private void saveFile(String dirPath, String name, Bitmap b, int quality) {
